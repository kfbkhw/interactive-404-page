...
export default function UFO() {
    const zeroRef = useRef<SVGPathElement>(null);
    const handleZeroAnimationEnd = () => {
        zeroRef.current?.classList.toggle('soak');
    };
        ...
        <>
            ...
            >
                <g id="UFO-wrap">
                    ...
                    <Zero
                        ref={zeroRef}
                        onAnimationEnd={handleZeroAnimationEnd}
                        id="number-0"
...
const rotateStar = keyframes`to {transform: rotateZ(360deg)}`;

const BigStar = styled.path`
    fill: #ffffff;
...
const twinkleStar = keyframes`0% {transform: scale(0.2)} 25% {transform: scale(1)} 50% {transform: scale(0.5)} 75% {transform: scale(1)} 100% {transform: scale(0)}`;

const LittleStar = styled.circle`
    fill: #ffffff;
...
const beamDown = keyframes`0% {transform: scaleX(0) scaleY(1.3) translateY(10px)} 60% {transform: scaleX(1) scaleY(1.3) translateY(10px)} 100% {transform: scaleX(0) scaleY(1.3) translateY(10px)}`;

const UFOBeam = styled.path`
    animation: ${beamDown} 5s infinite;
    transform-box: fill-box;
    transform-origin: center;
`;
...
const shakeUpAndDown = keyframes`0% {transform: translateY(0)} 25% {transform: translateY(5px)} 50% {transform: translateY(-5px)} 75% {transform: translateY(5px)} 100% {transform: translateY(0)}`;

const AlienHand = styled.path`
    animation: ${shakeUpAndDown} 1s infinite;
    transform-box: fill-box;
    transform-origin: center;
`;
